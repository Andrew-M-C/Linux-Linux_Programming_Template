/*******************************************************************************
	...
	File name: AMCBinSearchTree.c
	Description: 	
			This file implement functionalities in file AMCBinSearchTree.h.  
...
#define _CFG_DEFAULT_EXPECTED_NODE_COUNT		16
...
static int _sTree_DeleteNode(struct AMCStaticBinSearchTree *tree, struct AMCStaticBstNode *node, struct AMCStaticBstNode *parent)
{
	...
	{_DEBUG("#### MARK (%ld) ####", node->id);
		...
		_DEBUG(" ### small %ld <- %ld ###", smallestNode->id, parent->id);
...
static int _sTree_Delete(struct AMCStaticBinSearchTree *tree, const unsigned long id)
{
	...
	if (NULL == node)
	{
		return AMCBsTreeError_IdNotFound;
	}
...
struct AMCStaticBinSearchTree *AMCStaticBsTree_New(unsigned long contentSize, unsigned long expectedSize, int *errorOut)
{
	...
	if (NULL == ret)
	{
		_SET_ERROR(errorOut, AMCBsTreeError_SysCallError);
		return NULL;
	}
	else if (0 == contentSize)
	{
		free(ret);
		_SET_ERROR(errorOut, AMCBsTreeError_ParaError);
		return NULL;
	}
	else
	{}
	...
	if (expectedSize <= 1)
	{
		expectedSize = _CFG_DEFAULT_EXPECTED_NODE_COUNT;
	}
	...
	if (NULL == ret->memPool)
	{
		_SET_ERROR(errorOut, AMCBsTreeError_MemPoolError);
		free(ret);
		return NULL;
	}
...
int AMCStaticBsTree_Destroy(struct AMCStaticBinSearchTree *tree)
{
	if (NULL == tree)
	{
		return AMCBsTreeError_ParaError;
	}
...
int AMCStaticBsTree_Insert(struct AMCStaticBinSearchTree *tree, unsigned long id, const void *content)
{
	...
	{
		return AMCBsTreeError_BsTreeInvalid;
	}
	...
	if (NULL == node)
	{
		ret = AMCBsTreeError_MemPoolError;
	}
	else if (isDuplicated)
	{
		ret = AMCBsTreeError_IdExists;
	}
	else
	{
		if(content)
...
void *AMCStaticBsTree_GetContent(struct AMCStaticBinSearchTree *tree, unsigned long id, int *errorOut)
{
	...
	{
		_SET_ERROR(errorOut, AMCBsTreeError_IdNotFound);
		return NULL;
	}
}
...
int AMCStaticBsTree_ReadContent(struct AMCStaticBinSearchTree *tree, unsigned long id, void *contentOut, unsigned long bufferSize)
{
	...
	{
		return AMCBsTreeError_BufferTooSmall;
	}

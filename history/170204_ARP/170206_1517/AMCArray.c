...
static int _check_and_init_amc_array()
{
	if (NULL == g_item_mem_pool) {
		g_item_mem_pool = AMCMemPool_Create(sizeof(void*), g_item_init_size, g_item_inc_size, TRUE);
	}
...
struct AMCArrayItem *_object_at_index(AMCArray_st *array, size_t index)
{
	...
	/* determine the way for accccess */
	{
		...
		thisSearchCount = abs(array->count - index);
		if (thisSearchCount < predictSearchCount)
		{
			predictSearchCount = thisSearchCount;
		...
		_DB("%p: Request object at %d, cached %d, length %d, searh steps %d", 
			array, index, array->last_access_index, array->count, predictSearchCount);
	}
	...
	case _READ_FROM_END:
		{
			for (tmp = array->count, ret = array->last;
				tmp > index; 
				tmp--, ret = ret->prev)
			{}
...
AMCArrayErrno_st _append_object(AMCArray_st *array, void *object)
{
	...
	struct AMCArrayItem *lastItem = AMCMemPool_Alloc(g_item_mem_pool);
	if (NULL == lastItem) {
		return _make_error(AMC_ARRAY_MEMPOOL_ERROR);
	}

	_DB("%p: Append object, cached %d, length %d", array, array->last_access_index, array->count);

	/* set item value */
	lastItem->next = NULL;
	lastItem->prev = NULL;
	lastItem->value = object;
	...
	if (NULL == array->last) {
		...
		array->count = 1;
		array->last_access_index = 0;
		array->last_access_item  = lastItem;
	}
	else {
		array->last->next = lastItem;
		lastItem->prev = array->last;
...
AMCArrayErrno_st _insert_object(AMCArray_st *array, void *object, size_t index)
{
	...
	/* insert in the middle or in the end */
		...
		insItem->next = prevItem->next;
		insItem->prev = prevItem;
...
AMCArray_st *AMCArray_New(AMCArrayErrno_st *pErrOut)
{
	...
	{
		error.sys_errno = TO_INT16(errno);
		error.array_error = AMC_ARRAY_SYSTEM_ERROR;
		goto ENDS;
	}

	ret = AMCMemPool_Alloc(g_array_mem_pool);
	if (NULL == ret) {
		error.array_error = AMC_ARRAY_MEMPOOL_ERROR;
		goto ENDS;
	}

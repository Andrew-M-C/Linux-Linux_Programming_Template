...
int AMCDirCreate(const char *dirPath, mode_t mode)
{
	if (NULL == dirPath) {
		return -1;
	}
	...
	for (tmp = 0; (*pathChr) && (tmp < PATH_MAX); pathChr++, tmp++)
	{
		path[tmp] = *pathChr;
		if ('/' == *pathChr)
		{
			if (0 == stat(path, &dirStat))
			{
				...
				{
					AMCPrintErr("\"%s\" is not a directory", path);
					ret = -1;
					break;
				}
			}
			else
			{
				...
				if (ret < 0) {
					AMCPrintErr("Failed to create dir \"%s\": %s", path, strerror(errno));
					break;		/* !!! */
				}	
			}
		}
	...
	/* create the last level of directory */
	if (0 == ret)
	{
		ret = mkdir(dirPath, mode);
		if (ret < 0) {
			AMCPrintErr("Failed to create dir \"%s\": %s", dirPath, strerror(errno));
		}
	}
...
BOOL AMCAppIsRunning(int pid)
{
	...
	if (0 != kill(pid, 0))
	{
		...
		{
			/* system error */
			AMCPrintErr("Cannot stat process %d: %s", pid, strerror(errnoCopy));
			isProcAlive = FALSE;
		}
	}
	else
...
BOOL AMCAppAlreadyRunning(const char *pidDir, const char *pidFileName, int *pDuplicatePidOut)
{
	...
	if (AMCDirCreate(pidDir, S_IRWXU) < 0)
	{
		AMCPrintErr("Failed to create pid directory");
		ret = TRUE;
		goto ENDS;
	}

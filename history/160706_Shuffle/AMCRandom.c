...
/****/
/* ahead declarations */
#define __AHEAD_DECLARATIONS
#ifdef	__AHEAD_DECLARATIONS
...
/********/
#define __DEV_RANDOM_IMPLEMENTATIONS
#ifdef __DEV_RANDOM_IMPLEMENTATIONS


uint8_t AMCRealRandomByte()
{
	uint8_t ret = 0;
	ssize_t callStat = 0;
	char errorMsg[128] = "";
...
int AMCReadRandomByteArray(void *pArray, size_t nArraySize)
{
	if (NULL == pArray) {
		errno = EINVAL;
		return -1;
	...
	for (readCount = 0; readCount < nArraySize; readCount ++)
	{
		callStat = read(fd, pOut + readCount, sizeof(uint8_t));
		if (callStat < 0) {
			snprintf(errorMsg, sizeof(errorMsg), "Failed to read from ramdom device: %s\n", strerror(errno));
			goto ENDS;
		}
...
int32_t AMCRealRandomInt32(int32_t min, int32_t max)
{
	int32_t ret = 0;
	...
	else
	{
		int32_t tmp;
		int64_t result;
		double resultFloat, minFloat, maxFloat, tmpFloat;
		...
		tmp = 0;
		AMCReadRandomByteArray(&tmp, sizeof(tmp));
		...
		minFloat = min;
		maxFloat = max;
		tmpFloat = (double)((int64_t)tmp + 0x80000000);
		resultFloat = minFloat + (maxFloat - minFloat + 1.0) * (tmpFloat / ((double)0x100000000));

		ret = (int32_t)resultFloat;

		_DEBUG("(%.1f, %.1f) tmp = 0x%08lx (%d) (%.1f), result = %.1f", minFloat, maxFloat, tmp, tmp, tmpFloat, resultFloat);
	}

...
static void *_ev_watcher_alloc(size_t structSize, size_t argSize)
{
	void *ret = (argSize >= 0) ? malloc(structSize + argSize) : NULL;
	void **arg;
	size_t *argLen;
	if (ret)
	{		
		arg = (void**)(ret + structSize - sizeof(*argLen) - sizeof(*arg));
		argLen = (size_t*)(ret + structSize - sizeof(*argLen));
		*arg = argSize ? ((void*)(ret + structSize)) : NULL;
		*argLen = argSize;
	}
	return ret;
}
...
{
	return (struct AMC_ev_io*)_ev_watcher_alloc(sizeof(struct AMC_ev_io), argLen);
}

struct AMC_ev_timer *AMCEvTimer_new(size_t argLen)
{
	return (struct AMC_ev_timer*)_ev_watcher_alloc(sizeof(struct AMC_ev_timer), argLen);
}
struct AMC_ev_periodic *AMCEvPeriodic_new(size_t argLen)
{
	return (struct AMC_ev_periodic*)_ev_watcher_alloc(sizeof(struct AMC_ev_periodic), argLen);
}

struct AMC_ev_signal *AMCEvSignal_new(size_t argLen)
{
	return (struct AMC_ev_signal*)_ev_watcher_alloc(sizeof(struct AMC_ev_signal), argLen);
}

struct AMC_ev_child *AMCEvChild_new(size_t argLen)
{
	return (struct AMC_ev_child*)_ev_watcher_alloc(sizeof(struct AMC_ev_child), argLen);
}

struct AMC_ev_stat *AMCEvStat_new(size_t argLen)
{
	return (struct AMC_ev_stat*)_ev_watcher_alloc(sizeof(struct AMC_ev_stat), argLen);
}

struct AMC_ev_idle *AMCEvIdle_new(size_t argLen)
{
	return (struct AMC_ev_idle*)_ev_watcher_alloc(sizeof(struct AMC_ev_idle), argLen);
}

struct AMC_ev_prepare *AMCEvPrepare_new(size_t argLen)
{
	return (struct AMC_ev_prepare*)_ev_watcher_alloc(sizeof(struct AMC_ev_prepare), argLen);
}

struct AMC_ev_check *AMCEvCheck_new(size_t argLen)
{
	return (struct AMC_ev_check*)_ev_watcher_alloc(sizeof(struct AMC_ev_check), argLen);
}

struct AMC_ev_fork *AMCEvFork_new(size_t argLen)
{
	return (struct AMC_ev_fork*)_ev_watcher_alloc(sizeof(struct AMC_ev_fork), argLen);
}

struct AMC_ev_cleanup *AMCEvCleanup_new(size_t argLen)
{
	return (struct AMC_ev_cleanup*)_ev_watcher_alloc(sizeof(struct AMC_ev_cleanup), argLen);
}

struct AMC_ev_embed *AMCEvEmbed_new(size_t argLen)
{
	return (struct AMC_ev_embed*)_ev_watcher_alloc(sizeof(struct AMC_ev_embed), argLen);
}

struct AMC_ev_async *AMCEvAsync_new(size_t argLen)
{
	return (struct AMC_ev_async*)_ev_watcher_alloc(sizeof(struct AMC_ev_async), argLen);
}
...
struct AMC_ev_io *AMCEvIo_TcpListenNew(int port, int listenBacklog, size_t argLen)
{
	...
	struct AMC_ev_io *ret = AMCEvIo_new(argLen);
	int callStat = 0;
